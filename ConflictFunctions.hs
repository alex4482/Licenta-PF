...
solveConflict formula mem conflictLevel conflictClause =
  let faultyLiterals = findFaultyLiteralsInConflict conflictClause conflictLevel mem
   in let clausesForResolution = findClausesForRes formula faultyLiterals
       in let (newClause, newLevel) = resolution clausesForResolution conflictLevel faultyLiterals mem
...
resolution clauses conflictLevel faultyLiterals mem =
    let (finalClause, guessLevel) = resolution' clauses conflictLevel faultyLiterals (-1) mem in
        (head finalClause, guessLevel)
...
resolution' clauses conflictLevel faultyLiterals backTrackLevel mem =
    if length clauses == 1
        ...
        else let firstClause = head clauses in
                let (secondClause, literalToRemove) = findSecondClauseForResolution firstClause (tail clauses) faultyLiterals in
                    let (newClause, newGuessLevel ) = resoluteTwoClauses firstClause secondClause literalToRemove conflictLevel mem in
                        resolution' (remove secondClause (tail clauses)) conflictLevel faultyLiterals (max backTrackLevel newGuessLevel) mem
...
findSecondClauseForResolution firstClause otherClauses faultyLiterals =
    let firstClauseLiterals = getLiteralsFromClause firstClause in
      if null (firstClauseLiterals `intersect` faultyLiterals)
...
findSecondClauseForResolution' faultyLiterals (clause : otherClauses) =
  let clauseLiterals = getLiteralsFromClause clause in
    let foundLiterals = faultyLiterals `intersect` clauseLiterals in
      if null foundLiterals
...
resoluteTwoClauses cl1 cl2 pivot conflictLevel mem =
  let firstLiterals = remove (negateLit pivot) (getLiteralsFromClause cl1) in
    let secondLiterals = remove pivot (getLiteralsFromClause cl2) in
      if null firstLiterals
        then if null secondLiterals
              then error "error at resolution of 2 clauses"
              else if length secondLiterals == 1 then ( Lit (head secondLiterals), 0 )
                    else (makeClauseFromLiterals secondLiterals , highestLevelBelowConflict secondLiterals conflictLevel mem)
        else if null secondLiterals
              then  if length firstLiterals == 1 then ( Lit (head firstLiterals), 0 )
...
highestLevelBelowConflict literals conflictLevel mem = 
  let allLevels = map (getGuessLevelFromMem mem) literals in 
    getMaxLevelFrom ( removeLevelsBiggerThan allLevels conflictLevel)

getMaxLevelFrom :: [GuessLevel] -> GuessLevel
getMaxLevelFrom levels = foldr (\ x y -> if x >= y then x else y) (head levels) (tail levels)
